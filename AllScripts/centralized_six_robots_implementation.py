...
def callback_odom_sixth(odom):
    global Xr6, Xr6_init
    x6_init = Xr6_init[0,0]; y6_init = Xr6_init[1,0]; th6_init = Xr6_init[2,0];
    z6 = 0.0
    ...
    th6 = 2*np.arcsin(qz6)
    P6_local = np.array([[xr6], [yr6], [z6]]) # Xr6 in local frame
    phi6 = th6 + th6_init
    R_g6 = np.array([[np.cos(th6_init), -np.sin(th6_init), 0.0],
                     [np.sin(th6_init), +np.cos(th6_init), 0.0],
		             [0.0,          0.0,           1.0]])
    P6 = np.matmul(R_g6,P6_local) + np.array([[x6_init], [y6_init], [z6]])
    # print("P5:", P5)
    # P5 Position in global frame, R_g5: rotation of frame robot 5 in global frame
    Xr6 = np.array([[P6[0,0]], [P6[1,0]], [phi6]]) # pose of robot in global frame
    # print("Xr5: ", Xr5)
    # Xr = np.array([[yr], [xr], [2*np.arcsin(qz)]])
...
states = np.array([[x1], [y1], [theta1], [x2], [y2], [theta2], [x3], [y3], [theta3], [x4], [y4], [theta4], [x5], [y5], [theta5], [x6], [y6], [theta6]]); n_states = len(states)
# print("states: ", states)
...
controls = np.array([[v1],[omega1], [v2], [omega2], [v3], [omega3], [v4], [omega4], [v5], [omega5] , [v6], [omega6]]); n_controls = len(controls)
...
'lbx': np.concatenate((np.matlib.repmat(np.array([[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_min],[omega_min],[v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min],[v_min],[omega_min]]),N,1)), axis=0), \
'ubx': np.concatenate((np.matlib.repmat(np.array([[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max]]),N,1)), axis=0)}
# print("args: ", args)
...
#
Xr1_init = np.array([[+0.7], [+0.4], [-2.618]])
Xr2_init = np.array([[0.0], [0.8], [-1.57]])
Xr3_init = np.array([[-0.7], [+0.4], [-0.523]])
Xr4_init = np.array([[-0.7], [-0.4], [+0.523]])
Xr5_init = np.array([[0.0], [-0.8], [+1.57]])
Xr6_init = np.array([[0.7], [-0.4], [+2.618]])
#
...
# print("x0: ", x0)
# Reference point
xs = np.array([[-0.7], [-0.4], [-2.618], [+0.0], [-0.8], [-1.57], [+0.7], [-0.4], [-0.523],\
 [+0.7], [+0.4], [+0.523], [0.0], [0.8], [1.57] ,\
 [-0.7], [+0.4], [+2.618]])                                                        # Reference posture.
...
while (LA.norm(x0-xs) > 1e-1) and (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    ...
    t0, u0 = shift(T, t0, u)
    x0 = np.concatenate((np.concatenate((np.concatenate((\
    ...
    # print(time.clock(), u_cl[mpciter,0], u_cl[mpciter,1], u_cl[mpciter,2], u_cl[mpciter,3], u_cl[mpciter,4], u_cl[mpciter,5], u_cl[mpciter,6], u_cl[mpciter,7], u_cl[mpciter,8], u_cl[mpciter,9],u_cl[mpciter,10], u_cl[mpciter,11], Xr1, Xr2, Xr3, Xr4, Xr5, Xr6)
    print(time.clock(), round(u_cl[mpciter,0],3), round(u_cl[mpciter,1],3), round(u_cl[mpciter,2],3), \
    round(u_cl[mpciter,3],3), round(u_cl[mpciter,4],3), round(u_cl[mpciter,5],3), round(u_cl[mpciter,6],3), \
    round(u_cl[mpciter,7],3), round(u_cl[mpciter,8],3), round(u_cl[mpciter,9],3), \
    round(u_cl[mpciter,10],3), round(u_cl[mpciter,11],3))
