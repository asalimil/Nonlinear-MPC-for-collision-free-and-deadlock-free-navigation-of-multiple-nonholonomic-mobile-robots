...
states = np.array([[x1], [y1], [theta1], [x2], [y2], [theta2], [x3], [y3], [theta3]]); n_states = len(states)
# print("states: ", states)
...
controls = np.array([[v1],[omega1], [v2], [omega2], [v3], [omega3]]); n_controls = len(controls)
rhs = np.array([[v1*np.cos(theta1)],[v1*np.sin(theta1)],[omega1], [v2*np.cos(theta2)],[v2*np.sin(theta2)],[omega2], [v3*np.cos(theta3)],[v3*np.sin(theta3)],[omega3]])                   # system r.h.s
# print("rhs: ", rhs)
...
for k in range(N):
    ...
    g = vertcat(g, vertcat(vertcat(vertcat(st_next-st_next_euler, d12), d13), d23)) # compute constraints
...
'lbx': np.concatenate((np.matlib.repmat(np.array([[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_min],[omega_min],[v_min],[omega_min], [v_min],[omega_min]]),N,1)), axis=0), \
'ubx': np.concatenate((np.matlib.repmat(np.array([[+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max]]),N,1)), axis=0)}
# print("args: ", args)
...
sim_tim = 1000
t0 = 0;
x0 = np.array([[-1.0], [-1.0], [1.57], [0.0], [-1.0], [1.57], [+1.0], [-1.0], [1.57]])
Xr1 = np.array([[-1.0], [-1.0], [1.57]])
Xr2 = np.array([[+0.0], [-1.0], [1.57]])                                                            # initial condition.
Xr3 = np.array([[+1.0], [-1.0], [1.57]])
...
# print("x0: ", x0)
xs = np.array([[+2.0], [2.0], [0.0], [+2.0], [+1.0], [0.0], [+2.0], [0.0], [0.0]])                                                        # Reference posture.
...
while (LA.norm(x0-xs) > 5e-2) and (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    ...
    t0, u0 = shift(T, t0, u)
    x0 = np.concatenate((np.concatenate((Xr1, Xr2), axis=0), Xr3), axis=0)
    ...
    # print(time.clock(), u_cl[mpciter,0], u_cl[mpciter,1], u_cl[mpciter,2], u_cl[mpciter,3], u_cl[mpciter,4], u_cl[mpciter,5], Xr1, Xr2, Xr3)
    ...
    move3.angular.z = u_cl[mpciter,5]                                                    # apply first optimal angular velocity                                                    # apply first optimal angular velocity
    #
    pub1.publish(move1)
    pub2.publish(move2)
    pub3.publish(move3)
