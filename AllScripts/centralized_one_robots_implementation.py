...
while (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    # Goal points
    if goal_idx == 1:
        xs = np.array([[0.8382], [+0.3556], [0.785]]) # reference pose: (33 in, 14 in, 45 deg)
    elif goal_idx == 2:
        xs = np.array([[+0.0], [0.7112], [-1.57]]) # reference pose
    elif goal_idx == 3:
        xs = np.array([[-1.176], [+0.3556], [-3.14]]) # reference pose
    elif goal_idx == 4:
        xs = np.array([[-0.5588], [-0.7112], [+0.785]]) # reference pose
    elif goal_idx == 5:
        xs = np.array([[0.8382], [-0.7112], [-0.785]]) # reference pose
    elif goal_idx == 6:
