...
def callback_odom_fifth(odom):
    global Xr5, Xr5_init
    x5_init = Xr5_init[0,0]; y5_init = Xr5_init[1,0]; th5_init = Xr5_init[2,0];
    z5 = 0.0
    ...
    th5 = 2*np.arcsin(qz5)
    P5_local = np.array([[xr5], [yr5], [z5]]) # Xr5 in local frame
    phi5 = th5 + th5_init
    R_g5 = np.array([[np.cos(th5_init), -np.sin(th5_init), 0.0],
                     [np.sin(th5_init), +np.cos(th5_init), 0.0],
		             [0.0,          0.0,           1.0]])
    P5 = np.matmul(R_g5,P5_local) + np.array([[x5_init], [y5_init], [z5]])
    # print("P5:", P5)
    # P5 Position in global frame, R_g5: rotation of frame robot 5 in global frame
    Xr5 = np.array([[P5[0,0]], [P5[1,0]], [phi5]]) # pose of robot in global frame
    # print("Xr5: ", Xr5)
    # Xr = np.array([[yr], [xr], [2*np.arcsin(qz)]])
...
states = np.array([[x1], [y1], [theta1], [x2], [y2], [theta2], [x3], [y3], [theta3], [x4], [y4], [theta4], [x5], [y5], [theta5]]); n_states = len(states)
# print("states: ", states)
...
controls = np.array([[v1],[omega1], [v2], [omega2], [v3], [omega3], [v4], [omega4], [v5], [omega5]]); n_controls = len(controls)
rhs = np.array([[v1*np.cos(theta1)],[v1*np.sin(theta1)],[omega1], [v2*np.cos(theta2)],[v2*np.sin(theta2)],[omega2], [v3*np.cos(theta3)],[v3*np.sin(theta3)],[omega3], [v4*np.cos(theta4)],[v4*np.sin(theta4)],[omega4], [v5*np.cos(theta5)],[v5*np.sin(theta5)],[omega5]])                   # system r.h.s
# print("rhs: ", rhs)
...
Q[0,0] = 1; Q[1,1] = 5; Q[2,2] = 0.1; Q[3,3] = 1; Q[4,4] = 5; Q[5,5] = 0.1   #0.1                           # weighing matrices (states)
...
R[0,0] = 0.5; R[1,1] = 0.05; R[2,2] = 0.5; R[3,3] = 0.05       #0.05                                  # weighing matrices (controls)
R[4,4] = 0.5; R[5,5] = 0.05; R[6,6] = 0.5; R[7,7] = 0.05       #0.05
...
for k in range(N):
    ...
    g = vertcat(g, vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(st_next-st_next_euler, d12), d13), d14), d15), d23), d24), d25), d34), d35), d45) # compute constraints
...
'lbx': np.concatenate((np.matlib.repmat(np.array([[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_min],[omega_min],[v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min]]),N,1)), axis=0), \
'ubx': np.concatenate((np.matlib.repmat(np.array([[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max]]),N,1)), axis=0)}
# print("args: ", args)
...
#
Xr1_init = np.array([[0.0], [0.7112], [0.0]])
Xr2_init = np.array([[-0.2794], [0.3556], [0.0]])
Xr3_init = np.array([[-0.5588], [0.0], [0.0]])
Xr4_init = np.array([[-0.2794], [-0.3556], [0.0]])
Xr5_init = np.array([[0.0], [-0.7112], [0.0]])
#
...
# print("x0: ", x0)
xs = np.array([[+0.5588], [-0.7112], [0.0], [+0.8382], [-0.3556], [0.0], [+1.176], [+0.0], [0.0], \
[+0.8382], [+0.3556], [+0.0], [+0.5588], [+0.7112], [0.0]])                                                        # Reference posture.
...
# Main Loop
while (LA.norm(x0-xs) > 5e-1) and (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    ...
    t0, u0 = shift(T, t0, u)
    x0 = np.concatenate((np.concatenate((np.concatenate((np.concatenate((Xr1, Xr2), axis=0), Xr3), axis=0), Xr4), axis=0), Xr5), axis=0)
    ...
    # print(time.clock(),u_cl[mpciter,0], u_cl[mpciter,1], u_cl[mpciter,2], u_cl[mpciter,3], u_cl[mpciter,4], u_cl[mpciter,5], u_cl[mpciter,6], u_cl[mpciter,7], u_cl[mpciter,8], u_cl[mpciter,9], Xr1, Xr2, Xr3, Xr4, Xr5)
    print(time.clock(),round(u_cl[mpciter,0],3), round(u_cl[mpciter,1],3), round(u_cl[mpciter,2],3), round(u_cl[mpciter,3],3), \
    round(u_cl[mpciter,4],3), round(u_cl[mpciter,5],3), round(u_cl[mpciter,6],3), round(u_cl[mpciter,7],3), round(u_cl[mpciter,8],3),\
    round(u_cl[mpciter,9],3))
