...
def callback_odom_eighth(odom):
    ...
    th8 = 2*np.arcsin(qz8)
    # thr8 = modify(th8)
    Xr8 = np.array([[xr8], [yr8], [th8]])

def callback_odom_ninth(odom):
    global Xr9
    xr9 = odom.pose.pose.position.x
    yr9 = odom.pose.pose.position.y
    qz9 = odom.pose.pose.orientation.z
    qw9 = odom.pose.pose.orientation.w
    th9 = 2*np.arcsin(qz9)
    # thr9 = modify(th9)
    Xr9 = np.array([[xr9], [yr9], [th9]])

def callback_odom_tenth(odom):
    global Xr10
    xr10 = odom.pose.pose.position.x
    yr10 = odom.pose.pose.position.y
    qz10 = odom.pose.pose.orientation.z
    qw10 = odom.pose.pose.orientation.w
    th10 = 2*np.arcsin(qz10)
    # thr10 = modify(th10)
    Xr10 = np.array([[xr10], [yr10], [th10]])
#===============================================================================
...
sub9 = rospy.Subscriber('/tb3_9/odom', Odometry, callback_odom_ninth)
sub10 = rospy.Subscriber('/tb3_10/odom', Odometry, callback_odom_tenth)
...
x9 = SX.sym('x9'); y9 = SX.sym('y9'); theta9 = SX.sym('theta9')
x10 = SX.sym('x10'); y10 = SX.sym('y10'); theta10 = SX.sym('theta10')
...
states = np.array([[x1], [y1], [theta1], [x2], [y2], [theta2], [x3], [y3], [theta3], [x4], [y4], [theta4], [x5], [y5], [theta5] , [x6], [y6], [theta6] , [x7], [y7], [theta7] , [x8], [y8], [theta8] , [x9], [y9], [theta9] , [x10], [y10], [theta10]]); n_states = len(states)
# print("states: ", states)
...
v9 = SX.sym('v9'); omega9 = SX.sym('omega9')
v10 = SX.sym('v10'); omega10 = SX.sym('omega10')


controls = np.array([[v1],[omega1], [v2], [omega2], [v3], [omega3], [v4], [omega4], [v5], [omega5] , [v6], [omega6], [v7], [omega7], [v8], [omega8], [v9], [omega9], [v10], [omega10]]); n_controls = len(controls)
...
[v6*np.cos(theta6)],[v6*np.sin(theta6)],[omega6],[v7*np.cos(theta7)],[v7*np.sin(theta7)],[omega7],[v8*np.cos(theta8)],[v8*np.sin(theta8)],[omega8],[v9*np.cos(theta9)],[v9*np.sin(theta9)],[omega9],[v10*np.cos(theta10)],[v10*np.sin(theta10)],[omega10]  ])                   # system r.h.s
# print("rhs: ", rhs)
...
Q[24,24] = 1; Q[25,25] = 5; Q[26,26] = 0.1;
Q[27,27] = 1; Q[28,28] = 5; Q[29,29] = 0.1;
...
R[16,16] = 0.5; R[17,17] = 0.05
R[18,18] = 0.5; R[19,19] = 0.05
...
for k in range(N):
    ...
    d19 = (st[0]-st[24])**2 + (st[1]-st[25])**2
    d110 = (st[0]-st[27])**2 + (st[1]-st[28])**2
    #
    ...
    d29 = (st[3]-st[24])**2 + (st[4]-st[25])**2
    d210 = (st[3]-st[27])**2 + (st[4]-st[28])**2
    #
    ...
    d39 = (st[6]-st[24])**2 + (st[7]-st[25])**2
    d310 = (st[6]-st[27])**2 + (st[7]-st[28])**2
    #
    d45 = (st[9]-st[12])**2 + (st[10]-st[13])**2
    d46 = (st[9]-st[15])**2 + (st[10]-st[16])**2
    d47 = (st[9]-st[18])**2 + (st[10]-st[19])**2
    d48 = (st[9]-st[21])**2 + (st[10]-st[22])**2
    d49 = (st[9]-st[24])**2 + (st[10]-st[25])**2
    d410 = (st[9]-st[27])**2 + (st[10]-st[28])**2
    #
    d56 = (st[12]-st[15])**2 + (st[13]-st[16])**2
    d57 = (st[12]-st[18])**2 + (st[13]-st[19])**2
    d58 = (st[12]-st[21])**2 + (st[13]-st[22])**2
    d59 = (st[12]-st[24])**2 + (st[13]-st[25])**2
    d510 = (st[12]-st[27])**2 + (st[13]-st[28])**2
    #
    d67 = (st[15]-st[18])**2 + (st[16]-st[19])**2
    d68 = (st[15]-st[21])**2 + (st[16]-st[22])**2
    d69 = (st[15]-st[24])**2 + (st[16]-st[25])**2
    d610 = (st[15]-st[27])**2 + (st[16]-st[28])**2
    #
    d78 = (st[18]-st[21])**2 + (st[19]-st[22])**2
    d79 = (st[18]-st[24])**2 + (st[19]-st[25])**2
    d710 = (st[18]-st[27])**2 + (st[19]-st[28])**2
    #
    d89 = (st[21]-st[24])**2 + (st[22]-st[25])**2
    d810 = (st[21]-st[27])**2 + (st[22]-st[28])**2
    #
    d910 = (st[24]-st[27])**2 + (st[25]-st[28])**2
    ...
    g = vertcat(g, vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat( \
    vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(\
    vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(\
    vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(\
    vertcat(vertcat(vertcat(vertcat(vertcat(vertcat(st_next-st_next_euler, d12), d13), \
    d14), d15), d16), d17), d18), d19), d110), d23), d24), d25), d26), d27), d28), d29), d210), \
    d34), d35), d36), d37), d38), d39), d310), d45), d46), d47), d48), d49), d410), \
...
'lbx': np.concatenate((np.matlib.repmat(np.array([[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf],[-10],[-10],[-np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_min],[omega_min],[v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min], [v_min],[omega_min],[v_min],[omega_min],[v_min],[omega_min],[v_min],[omega_min],[v_min],[omega_min],[v_min],[omega_min]]),N,1)), axis=0), \
'ubx': np.concatenate((np.matlib.repmat(np.array([[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf], [+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf],[+10],[+10],[+np.Inf]]),N+1,1),np.matlib.repmat(np.array([[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max],[v_max],[omega_max]]),N,1)), axis=0)}
# print("args: ", args)
...
sim_tim = 1000
t0 = 0;
x0 = np.array([[-1.0], [+1.0], [-0.785], [+1.0], [+1.0], [-2.356], [+1.0], [-1.0], [+2.356], [-1.0], [-1.0], [0.785], [0.0], [0.0], [0.0], [+1.0], [+1.0], [-2.356], [+1.0], [+1.0], [-2.356], [+1.0], [+1.0], [-2.356], [+1.0], [+1.0], [-2.356], [+1.0], [+1.0], [-2.356]])
Xr1 = np.array([[-1.0], [+1.0], [-0.785]])
Xr2 = np.array([[+1.0], [+1.0], [-2.356]])                                                            # initial condition.
Xr3 = np.array([[+1.0], [-1.0], [+2.356]])
Xr4 = np.array([[-1.0], [-1.0], [+0.785]])                                                            # initial condition.
Xr5 = np.array([[0.0], [0.0], [0.0]])
Xr6 = np.array([[-1.0], [+1.0], [-0.785]])
Xr7 = np.array([[+1.0], [+1.0], [-2.356]])                                                            # initial condition.
Xr8 = np.array([[+1.0], [-1.0], [+2.356]])
Xr9 = np.array([[-1.0], [-1.0], [+0.785]])                                                            # initial condition.
Xr10 = np.array([[0.0], [0.0], [0.0]])
...
# print("x0: ", x0)
# Reference point
xs = np.array([[-1.5], [+1.0], [1.57], [-0.5], [+1.0], [1.57], [+0.5], [+1.0], [+1.57], [+1.5], [+1.0], [+1.57], [+2.5], [+1.0], [+1.57] ,\
 [-1.5], [-1.0], [-1.57], [-0.5], [-1.0], [-1.57], [+0.5], [-1.0], [-1.57], [+1.5], [-1.0], [-1.57], [2.5], [2.5], [0.0]])                                                        # Reference posture.
...
while (LA.norm(x0-xs) > 1e-1) and (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    ...
    np.concatenate((np.concatenate((np.concatenate((np.concatenate(\
    (Xr1, Xr2), axis=0), Xr3), axis=0), Xr4), axis=0), Xr5), axis=0), Xr6), axis=0)\
    , Xr7), axis=0), Xr8), axis=0), Xr9), axis=0), Xr10), axis=0)
    ...
    move8 = Twist()
    pub9 = rospy.Publisher('/tb3_9/cmd_vel', Twist, queue_size=10)
    move9 = Twist()
    pub10 = rospy.Publisher('/tb3_10/cmd_vel', Twist, queue_size=10)
    move10 = Twist()



    print("v1, w1, v2, w2, v3, w3, v4, w4, v5, w5, v6, w6, v7, w7, v8, w8, v9, v9, v10, w10: ", u_cl[mpciter,0], u_cl[mpciter,1], u_cl[mpciter,2], u_cl[mpciter,3], u_cl[mpciter,4], u_cl[mpciter,5], u_cl[mpciter,6], u_cl[mpciter,7], u_cl[mpciter,8], u_cl[mpciter,9],u_cl[mpciter,10], u_cl[mpciter,11], u_cl[mpciter,12], u_cl[mpciter,13], u_cl[mpciter,14], u_cl[mpciter,15], u_cl[mpciter,16], u_cl[mpciter,17], u_cl[mpciter,18], u_cl[mpciter,19])
    ...
    move9.linear.x = u_cl[mpciter,16]                                                     # apply first optimal linear velocity
    move9.angular.z = u_cl[mpciter,17]                                                    # apply first optimal angular velocity
    move10.linear.x = u_cl[mpciter,18]                                                     # apply first optimal linear velocity
    move10.angular.z = u_cl[mpciter,19]
    #
    pub1.publish(move1)
    pub2.publish(move2)
    pub3.publish(move3)
...
move8.angular.z = 0.0
move9.linear.x = 0.0                                                     # apply first optimal linear velocity
move9.angular.z = 0.0
move10.linear.x = 0.0                                                     # apply first optimal linear velocity
move10.angular.z = 0.0
#
pub1.publish(move1)
pub2.publish(move2)
pub3.publish(move3)
