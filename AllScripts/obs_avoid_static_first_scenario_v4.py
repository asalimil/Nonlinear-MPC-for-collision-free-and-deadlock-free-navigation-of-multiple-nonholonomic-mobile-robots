...
def callback_lidar(lidar):
    ...
    # Scan = np.transpose(np.reshape(Scan,-1,1))
    numRays = len(Scan)
...
# T = 0.075, N = 125
# T = 0.2, N = 100, Nc = 25  *OK
# T = 0.075, N = 150, Nc = 25 *OK
T = 0.075                                                                        # [s]
...
#=== Lower/Upper bounds
robot_radius = 0.15
x_min = -10.0; x_max = 10.0; y_min = -10.0; y_max = +10.0; theta_min = -np.Inf; theta_max = np.Inf
d_min = robot_radius; d_max = 10.0
#
...
L = 0.1*np.eye(numRays)

for k in range(N):
    st_d = X[n_statesX:,k]
    st_x = X[0:n_statesX,k]
    if k < Nc:
        con = U[:,k]
    else:
        con = U[:,Nc-1]
    ...
    obj = obj  +  mtimes((st_x-P[3:6]).T,mtimes(Q,(st_x-P[3:6]))) + mtimes(con.T, mtimes(R, con)) +\
    + mtimes(1/st_d.T, mtimes(L, 1/st_d))               # calculate obj
...
OPT_variables = vertcat(vertcat(vertcat(reshape(X,n_states*(N+1),1),reshape(U,n_controls*Nc,1))))
...
#
lowerbound_U = np.matlib.repmat(np.array([[v_min],[omega_min]]),Nc,1)
#
...
#
upperbound_U = np.matlib.repmat(np.array([[v_max],[omega_max]]),Nc,1)
...
u0 = np.zeros((n_controls,Nc));                                                  # two control inputs for each robot
...
while (not rospy.is_shutdown()):
    ...
    #=== Goal points
    if goal_idx == 1:
        xs = np.array([[3.0], [+2.5], [0.0]]) # reference pose
    elif goal_idx == 2:
        xs = np.array([[+0.0], [2.5], [-0.785]]) # reference pose
    elif goal_idx == 3:
        xs = np.array([[0.5], [-1.0], [0.0]]) # reference pose
    elif goal_idx == 4:
        xs = np.array([[-0.5], [-1.75], [+0.785]]) # reference pose
    elif goal_idx == 5:
        xs = np.array([[1.25], [-0.75], [-0.785]]) # reference pose
    elif goal_idx == 6:
    ...
    u0_reSh = reshape(np.transpose(u0),n_controls*Nc,1)
    ...
    solu = sol['x'][n_states*(N+1):n_states*(N+1)+n_controls*Nc]        # extract optimal u*
    solu_full = np.transpose(solu.full())
    u = np.transpose(reshape(solu_full, n_controls,Nc))                            # get controls only from the solution
    ...
    mpciter = mpciter + 1
    x0[n_statesX:n_statesX+numRays] = np.reshape(Scan, (-1,1))
    x0[0:n_statesX] = Xr
    # print("Scan: ", Scan)
