...
obj = 0                                                                        # Objective function
                                                                        # constraints vector
Q = np.zeros((6,6)); Q[0,0] = 1; Q[1,1] = 5; Q[2,2] = 0.1; Q[3,3] = 1; Q[4,4] = 5; Q[5,5] = 0.1                           # weighing matrices (states)
R = np.zeros((4,4)); R[0,0] = 0.5; R[1,1] = 0.05; R[2,2] = 0.5; R[3,3] = 0.05                                  # weighing matrices (controls)
...
g = st-P[0:6]                                                                   # initial condition constraints
#print("g: ", g)
...
args = {'lbg': np.zeros((1,6*(N+1))), 'ubg': np.zeros((1,6*(N+1))), \
...
Xr1 = np.array([[-2.0], [-1.0], [0.0]])
Xr2 = np.array([[2.5], [0.0], [0.0]])                                                            # initial condition.
#
print("Xr1: ", Xr1)
print("Xr2: ", Xr2)
...
# print("x0: ", x0)
xs = np.array([[+1.0], [0.0], [0.0], [3.0], [1.0], [0.0]])                                                        # Reference posture.
...
while (LA.norm(x0-xs) > 5e-2) and (not rospy.is_shutdown()):
 # and (mpciter < sim_tim / T)
    ...
    print("v1, w1, v2, w2: ", u_cl[mpciter,0], u_cl[mpciter,1], u_cl[mpciter,2], u_cl[mpciter,3])
